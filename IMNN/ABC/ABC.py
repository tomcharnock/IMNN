...
import numpy as np
import tqdm
from ..utils.utils import isnotebook
...
class ABC():
    ...
    __________
    sess : objTF
        information maximising neural network session
    prior : class
    ...
    n_summaries : int
        the number of summaries output by the network
    n_params : int
        the number of parameters in the model
    dictionary : dict
        dictionary to pass to network containing any extra necessary graph
        parameters
    ABC_dict : dict
    ...
    def ABC(self, draws, at_once=True, save_sims=None, return_dict=False,
        ...
        summaries : ndarray
            the summaries of the simulations from the IMNN
        differences : ndarray
        ...
        if return_dict:
            ...
            self.ABC_dict["summaries"] = np.concatenate(
                [self.ABC_dict["summaries"], summaries])
    ...
    def PMC(self, draws, posterior, criterion, at_once=True, save_sims=None,
        ...
        accepted_summaries : ndarray
            the summaries which have successfully moved closer than epsilon.
        ...
        if self.total_draws == 0 or restart:
            ...
            self.PMC_dict["summaries"] = self.PMC_dict[
                "summaries"][inds[:posterior]]
        ...
        while criterion < criterion_reached:
            ...
            while current_draws > 0:
                ...
                accepted_summaries[move_ind[accept_index]] = \
